...
12. CountDownLatch 你知道吗？说一下。信号量呢？ (阿里Lazada、京东)
> CountDownLatch 倒计时计数器，不像 Semaphore 能通过release来恢复，CountDownLatch countDown()是一直数到0 不可恢复的
...
> 信号量 控制某个资源 只能同时被访问的线程数 acquire() release() 来控制，availablePermits() 可以看目前有多少可用的 permits
>     使用场景，和线程池的区别
...
> 原子性：对其操作 要么全部成功 要么全部失败
> 可见性：在修改结束的时候马上刷新到主存，其他线程可以马上知道最新的值
> 禁止指令重排：
